...
int *propagate_actionNet(float weight_action[LAYERS][CONNECTIONS], int input[INPUTA]){
    ...
    for(i=0; i<OUTPUTA; i++){ // outputs
        ...
        for(j=0;j<HIDDENA;j++){ // hidden layers
            net[i] += hidden_net[j]*weight_action[2][HIDDENA*i+j];
        }
...
void propagate_predictionNet(float weight_prediction[LAYERS][CONNECTIONS], int a, int input[INPUTP]) {
    ...
    for(i=0;i<OUTPUTP;i++) { //outputs (== # of sensors)
        ...
        for(j=0;j<HIDDENP;j++) { // hidden layers
            net[i] += hidden_net[j]*weight_prediction[2][i*HIDDENP+j];
        }
        net[i] = activation(net[i]);
    }
...
float doRun(int gen, int ind, struct agent p_initial[], int maxTime, int log, int noagents){
    ...
    sprintf(str, "%d_%d_%d_%d_%d", COUNT, 0, no_agents, SIZE_X, SIZE_Y);
    ...
    for(i=0; i<no_agents; i++){
        ...
        if(MANIPULATION != NONE){
            ...
            // else if DISPERSION - all 0
            else if(p[i].type == AGGREGATION){
                for(j=0; j<SENSORS; j++){ // set all to 1
                    predictions[i][j] = 1;
                }
            }
        }
    ...
    while(timeStep < maxTime){
        ...
        for(i=0;i<no_agents;i++){
            ...
            if(current_action[i][timeStep] == STRAIGHT){ // move 1 grid cell straight
                ...
                p_next[i].heading.x = round(cos(angle + action_output[1]*(PI/2)));
                p_next[i].heading.y = round(sin(angle + action_output[1]*(PI/2)));
            }
        } // agents
...
void evolution(){
    ...
    // file names
    sprintf(str, "%d_%d_%d", COUNT, 0, NUM_AGENTS);
...
int main(int argc, char** argv){
    ...
    if(!strcmp(argv[1], "REPLAY")){
        ...
        EVOL = 0;
        if(argc != 18){
            printf("Please specify 17 input values: [REPLAY/EVOL] [path to agent_trajectory file] [path to prediction_weights file] [path to action_weights file] [Fitness Function] [Manipulation: NONE/PRE/MAN] [Agent Type][REMOVE][DESTROY][New # agents][# of agents to be copied][x_min (destroy)][x_max][y_min][y_max][SIZE_X][SIZE_Y] \n");
            exit(0);
        }
    }
    ...
    if(EVOL){
        ...
        if(MANIPULATION != NONE){
            ...
            else if(!strcmp(argv[6], "AGGREGATION")){
                type1 = AGGREGATION;
            }
            else if(strcmp(argv[6], "DISPERSION")){
                type1 = DISPERSION;
            }
            else{
        ...
        // do evolutionary run w/ all agents 
        evolution();
    }
        ...
        if(MANIPULATION != NONE){
            ...
            else if(!strcmp(argv[7], "AGGREGATION")){
                type1 = AGGREGATION;
            }
            else if(!strcmp(argv[7], "DISPERSION")){
                type1 = DISPERSION;
            }
            else{
                printf("Unknown Agent Type.\n");
        ...
        if(MANIPULATION != PRE){
            ...
            if (file != NULL) {
                ...
                while (fgets(line, sizeof(line), file)) {
                    i++;
                    if(i == ((MAX_GENS-1)*4) + 1){ // 397 for 100 Gen, 1 for 1 Gen
                        break;
                    }
                }
        ...
        fprintf(f, "Fitness of re-run: %e\n", doRun(0, 0, initial, MAX_TIME, 0, NUM_AGENTS_OLD));
        ...
        // set new size according to parameters
        if(atof(argv[16]) > 0 && atof(argv[17]) > 0){
            SIZE_X = atof(argv[16]);
            SIZE_Y = atof(argv[17]);
        }
        ...
        if(!DESTROY){
            ...
            for(i=copy_agent_no; i<new_agent_no; i++){
                ...
                while(b){
                    ...
                    if(grid[replay[i].coord.x][replay[i].coord.y] == 0){ // not occupied
                        b = 0;
                        grid[replay[i].coord.x][replay[i].coord.y] = 1; // set grid cell occupied
                    }
                }
            ...
            if(!REMOVE){ // if not remove - replace agents to new positions 
                for(j=j; j<new_agent_no; j++){
                    ...
                    while(b){
                        ...
                        if(grid[replay[j].coord.x][replay[j].coord.y] == 0){
                            b = 0;
                        }
                    }
        ...
        fprintf(f, "Fitness of self-repair [%d, %d]: %e\n", SIZE_X, SIZE_Y, doRun(0, 0, replay, MAX_TIME, 1, new_agent_no));
