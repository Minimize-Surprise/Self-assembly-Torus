...
def measure_grouping(file, timesteps="500"):
	global grid 
	global agents 
	global noAggregate
	global aggregate
	...
	while pos: 
		...
		if cnt >= 6: 
			...
			while tmp: # check neighbors of neighbors 
				current, cnt = check_neighbors(all_agents_without[tmp[0]])
	...
	return (float(len(flat_list))/float(agents)), key
...
if __name__ == "__main__":
	...
	measure_grouping(file)

	# calculate & print diff 
	f = open('cluster_aggregation_loose_grouping.txt', 'w')
