...
def measure_square(file,timesteps="500"):
	global agents 
	global grid 
	global square
	global noSquare 
	...
	for line in f: 	
		...
		if line[:4] == "Gen:":
			...
			p = row[1].split(', ')
			pos.append([[int(p[0]), int(p[1])], 0]) #heading unimportant for squares lattice 
	...
	for el in elements:
		...
		# rest of agents - cells to be empty 
	...
	return noSquare, (float(len(square))/float(agents)) 
