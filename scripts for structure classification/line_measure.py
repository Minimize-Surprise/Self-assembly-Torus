...
def check_consecutive(L, max_val):
	# if line connects over torus border 
	if 0 in L and max_val in L:
		return True 
...
def measure_line(file, timesteps="500"): 
	global grid 
	global agents 
	global lines 
	global noLine 
	...
	while len(pos) > 1:	
		...
		if line[-1][1][0] != 0: # heading in x-direction 
			...
			if len(line) == grid[0]:
				...
				neighbors.sort()
				if check_consecutive(neighbors, (grid[0]-1)):
					lineflag = 1
			...
			if len(line) == grid[1]: # line length = grid length 
				...
				if neighbor_count_top > max_neighbors or check_consecutive(neighbors, (grid[1]-1)):
					lineflag = 1 
				...
				if neighbor_count_bottom > max_neighbors or check_consecutive(neighbors, (grid[1]-1)):
					lineflag = 1 
	...
	return noLine, (float(agents-len(noLine))/float(agents))
